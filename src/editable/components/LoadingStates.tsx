...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse bg-[#eae5df]', className)} />
}

export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-[1320px] px-4 py-16 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#FF6B00]">{label}</p>
      <PulseBlock className="mt-6 h-14 w-3/4 max-w-2xl" />
...
export function DetailLoadingState({ label = 'Loading detail', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto grid w-full max-w-[1320px] gap-8 px-4 py-16 lg:grid-cols-[0.8fr_1.2fr]', className)} aria-live="polite" aria-busy="true">
      <PulseBlock className="h-[480px] w-full" />
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#FF6B00]">{label}</p>
